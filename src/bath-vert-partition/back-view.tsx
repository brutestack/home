...
export function BackView({ onMouseMove, mouse }: BackViewProps) {
  ...
  // Общая длина с колонной (зеркально — стена слева, колонна справа)
  const totalLen = BATH_VERT_LEN_MM + COL_W_MM;
  ...
  // Конец перегородки (перед колонной)
  const partEnd = BATH_VERT_LEN_MM;
  const colStart = BATH_VERT_LEN_MM;
  ...
  return (
    <svg viewBox={`0 0 ${svgW} ${svgH}`}
      ...
      <text x={svgW/2} y={24} textAnchor="middle" fill={C_COLUMN_TEXT} fontSize={14} fontWeight="bold">
        Л6.Сх2 — Вид со стороны спальни (шаг стоек {STUD_STEP_MM} мм)
      </text>
      ...
      {/* Нижняя стена (слева) — обозначение */}
      <rect x={p} y={p} width={10} height={CEILING_H_MM * s}
        fill={C_TEXT_DIM} stroke={C_TEXT} strokeWidth={1}/>
      <text x={p - 5} y={p + CEILING_H_MM/2 * s}
        textAnchor="middle" fill={C_TEXT} fontSize={8} transform={`rotate(-90,${p - 5},${p + CEILING_H_MM/2 * s})`}>стена</text>

      {/* Колонна 1 (справа) */}
      <rect x={p + colStart * s} y={p} width={COL_W_MM * s} height={CEILING_H_MM * s}
        fill={C_COL_FILL} stroke={C_COLUMN} strokeWidth={2}/>
      <text x={p + (colStart + COL_W_MM/2) * s} y={p + CEILING_H_MM/2 * s}
        textAnchor="middle" fill={C_COLUMN} fontSize={10} fontWeight="bold">Колонна 1</text>
      <text x={p + (colStart + COL_W_MM/2) * s} y={p + CEILING_H_MM/2 * s + 14}
        ...
        return (
          <g key={`stud${i}`}>
            <rect x={p + mPos * s} y={p + PN_H_MM * s}
              ...
              stroke={C_FRAME}
              strokeWidth={1}/>
            <StudLabel x={p + (mPos + BATH_VERT_PS_W_MM/2) * s} y={p + CEILING_H_MM/2 * s} num={getBathVertStudNumber(i)} color={C_FRAME}/>
            <SpecLabel x={p + (mPos + BATH_VERT_PS_W_MM/2) * s} y={p + CEILING_H_MM/2 * s + 40} num={7} color={C_FRAME}/>
          </g>
        );
      })}

      {/* ГКЛ раскладка смещена на 600 мм (вразбежку) */}
      {/* ГКЛ доборка у стены (слева) */}
      <rect x={p} y={p + DOBOR_H_MM * s}
        width={(STUD_STEP_MM) * s} height={GKL_SHEET_H_MM * s}
        fill={C_GKL_PANEL_FILL} stroke={C_GKL_PANEL} strokeWidth={2}/>
      <text x={p + STUD_STEP_MM/2 * s} y={p + (DOBOR_H_MM + GKL_SHEET_H_MM/2) * s}
        textAnchor="middle" fill={C_GKL_PANEL} fontSize={8}>{STUD_STEP_MM}</text>
      <SpecLabel x={p + STUD_STEP_MM/2 * s} y={p + (DOBOR_H_MM + 30)} num={4} color={C_GKL_PANEL}/>

      {/* ГКЛ лист 1 */}
      <rect x={p + STUD_STEP_MM * s} y={p + DOBOR_H_MM * s}
        width={GKL_SHEET_W_MM * s} height={GKL_SHEET_H_MM * s}
        fill={C_GKL_PANEL_FILL} stroke={C_GKL_PANEL} strokeWidth={2}/>
      <text x={p + (STUD_STEP_MM + GKL_SHEET_W_MM/2) * s} y={p + (DOBOR_H_MM + GKL_SHEET_H_MM/2) * s}
        textAnchor="middle" fill={C_GKL_PANEL} fontSize={9}>ГКЛ {GKL_SHEET_W_MM}×{GKL_SHEET_H_MM}</text>
      <SpecLabel x={p + (STUD_STEP_MM + GKL_SHEET_W_MM/2) * s} y={p + (DOBOR_H_MM + 30)} num={1} color={C_GKL_PANEL}/>

      {/* ГКЛ остаток у колонны */}
      {(() => {
        const remain = BATH_VERT_LEN_MM - STUD_STEP_MM - GKL_SHEET_W_MM;
        return (
          <>
            <rect x={p + (STUD_STEP_MM + GKL_SHEET_W_MM) * s} y={p + DOBOR_H_MM * s}
              width={remain * s} height={GKL_SHEET_H_MM * s}
              fill={C_GKL_PANEL_FILL} stroke={C_GKL_PANEL} strokeWidth={2}/>
            <text x={p + (STUD_STEP_MM + GKL_SHEET_W_MM + remain/2) * s} y={p + (DOBOR_H_MM + GKL_SHEET_H_MM/2) * s}
              textAnchor="middle" fill={C_GKL_PANEL} fontSize={8}>{remain}</text>
            <SpecLabel x={p + (STUD_STEP_MM + GKL_SHEET_W_MM + remain/2) * s} y={p + (DOBOR_H_MM + 30)} num={2} color={C_GKL_PANEL}/>
          </>
        );
      })()}
      ...
      <rect x={p} y={p}
        ...
        fill={C_GKL_PANEL_FILL} stroke={C_GKL_PANEL} strokeWidth={2}/>
      <text x={p + BATH_VERT_LEN_MM/2 * s} y={p + DOBOR_H_MM/2 * s + 3}
        textAnchor="middle" fill={C_GKL_PANEL} fontSize={8}>ГКЛ доборка {BATH_VERT_LEN_MM}×{DOBOR_H_MM}</text>
      <SpecLabel x={p + BATH_VERT_LEN_MM/2 * s + 100} y={p + DOBOR_H_MM/2 * s} num={3} color={C_GKL_PANEL}/>

      {/* Размерные линии */}
      <HDim x1={p} x2={p + BATH_VERT_LEN_MM * s} y={p - 20}
        label={BATH_VERT_LEN_MM} fontSize={10}/>
      <HDim x1={p + colStart * s} x2={p + totalLen * s} y={p - 20}
        color={C_COLUMN} textColor={C_COLUMN} label={COL_W_MM} fontSize={8}/>

      {/* Размерные линии листов ГКЛ */}
      <HDim x1={p} x2={p + STUD_STEP_MM * s} y={p + CEILING_H_MM * s + 20}
        color={C_GKL_PANEL} textColor={C_GKL_PANEL} label={STUD_STEP_MM} fontSize={8}/>
      <HDim x1={p + STUD_STEP_MM * s} x2={p + (STUD_STEP_MM + GKL_SHEET_W_MM) * s} y={p + CEILING_H_MM * s + 20}
        color={C_GKL_PANEL} textColor={C_GKL_PANEL} label={GKL_SHEET_W_MM} fontSize={8}/>
      {(() => {
        const remain = BATH_VERT_LEN_MM - STUD_STEP_MM - GKL_SHEET_W_MM;
        return (
          <HDim x1={p + (STUD_STEP_MM + GKL_SHEET_W_MM) * s} x2={p + BATH_VERT_LEN_MM * s} y={p + CEILING_H_MM * s + 20}
            color={C_GKL_PANEL} textColor={C_GKL_PANEL} label={remain} fontSize={8}/>
        );
      })()}
